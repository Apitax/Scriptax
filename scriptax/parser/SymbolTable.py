...
class SymbolTable(GenericTable):
    ...
    # Executes a method by getting the method body, and its appropriate static parent and birthing a scope for the
    #   method to operate within
    ...
    def _get_import(self, name) -> Symbol:
    ...
    def copy(self, import_name, var_name) -> Symbol:
    ...
    # Returns the SymbolTable of the extended scope (the copy of the imported scope)
    #    in order to parse using it
    def extends(self, import_name) -> 'ExtendedImport':
        # If we imported module exists, then get the symbol for it
        symbol: Symbol = self._get_import(import_name)
